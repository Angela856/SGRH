package cl.duocuc.dsy1103.servicioextra.client;
...
public class ReservaClient {
    ...
    public Object obtenerReservaPorId(Long idReserva) {
        try {
            log.info("Client: Verificando validez de la reserva con ID: {}", idReserva);
            return reservaRestClient.get()
                    .uri("/api/reservas/{id}", idReserva)
                    .retrieve()
                    .body(Object.class); // Mapeo genérico para evitar problemas de imports cruzados
        } catch (Exception e) {
            log.error("Error HTTP al conectar con el microservicio de Reserva para ID {}: {}", idReserva, e.getMessage());
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, 
                    "No se puede asociar el servicio adicional. La reserva con ID '" + idReserva + "' no existe.");
        }
    }
}
